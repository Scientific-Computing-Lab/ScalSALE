...
contains
    ...
    type(hydro_step_t) function Constructor (df, nx, ny, nz, nxp, nyp, nzp, wilkins_scheme, mesh, velocity, acceleration,&
        ...
        nmats, materials, num_mat_cells, mat_id, emf, emfm, parallel_params, mat_ids, use_GPU)
        implicit none
    ...
    subroutine do_time_step_3d(this, time)
        ...
        call this%Calculate_thermodynamics() ! calculate volume
        ...
        do_offload=this%use_GPU
        !$omp target update if(do_offload) to(x_target, y_target, z_target, velocity_x_target, velocity_y_target, velocity_z_target, vof_target, vol_target)

        call time%Calculate_dt(this%mesh, this%velocity, this%rezone%mesh_velocity, this%vertex_mass, this%total_vof, this%emfm) !calculate dt
        this%cyc_delete = this%cyc_delete+1

        call this%Calculate_acceleration_3d(time%dt_mid)! here we update acceleartion quantity -- from x,y,z

        call this%Calculate_artificial_viscosity_3d(time) ! here we update artificial viscosity FROM acceleartion

        call this%Calculate_velocity_3d(time%dt_mid) ! calculate velocity from acceleration and avisc
        ! after we calculated velocity WE "CAN" update the derivatives. with x,y,z, vof, vol, velocity

        call this%rezone%Calculate_rezone_3d(time%dt)! we update velocity again
        
        call this%Calculate_energy_3d(this%init_temperature, time%dt) ! calcualt energy without velocity or coordinates

        call this%Fix_vof_3d(time%dt, 0) ! we dont really do anything 

        call this%Calculate_mesh_3d(time) ! we calculate density and volume and coordinates

        if (this%rezone%rezone_type /= 0) call this%advect%Calculate_advect_3d() ! this changes everything
...
!        stop
    ...
    subroutine Calculate_acceleration_3d(this, dt_mid)
        ...
        call this%velocity%Calculate_derivatives(this%mesh%coordinates, &
            this%total_vof, this%total_volume, this%nx, this%ny, this%nz, this%emf, this%use_GPU)
    ...
    subroutine Calculate_velocity_3d(this, dt_mid)
        ...
        call this%velocity%Calculate_derivatives(this%mesh%coordinates, this%total_vof, this%total_volume&
            , this%nx, this%ny, this%nz, this%emf, this%use_GPU)
...
!            end do
!        end do
!yonifk: reem told to remove ! call this%velocity%Calculate_derivatives(this%mesh%coordinates, &
!            this%total_vof, this%total_volume, this%nx, this%ny, this%nz, this%emf, this%use_GPU)
call this%total_sie%exchange_end()
call this%materials%sie%exchange_end()
