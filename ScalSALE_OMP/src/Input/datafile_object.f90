...
contains
    type(datafile_t) function Constructor(datafile)
        ...
        call Constructor%Parse_GPU(json)
    ...
    subroutine Parse_GPU(this, json)
        implicit none
        class (datafile_t) , intent(inout) :: this
        type(json_file)    , intent(inout) :: json

        character(len=*), parameter    :: segment  = "GPU"
        character(len=255) :: tmp
        character(len=255) :: root
        logical :: found, var_l
        logical :: var
        tmp = segment // ".use_GPU"
        call json%get(trim(tmp), var, found)
        if (found .eqv. .false.) call error_msg("Bad value in GPU segment")
        this%use_GPU= var

    end subroutine Parse_GPU
