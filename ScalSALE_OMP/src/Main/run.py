...
def runner(df):
    subprocess.Popen(['source ../Scripts/module_load.sh'], shell=True, stdout=subprocess.PIPE)

    f = open(df)
    j_object = json.load(f)
    np = get_np(j_object)
    f.close()
    a = os.system('mpirun -n {0} --mca btl self,sm,openib python ../Main/main.py -df {1} '.format(np, df))
