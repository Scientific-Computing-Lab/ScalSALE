...
contains
    ...
    type(problem_t) function Constructor(df)
        ...
        nc = 1
        Constructor%use_GPU = df%use_GPU
        ...
        Constructor%hydro = hydro_step_t(df, Constructor%nx , Constructor%ny , Constructor%nz, Constructor%nxp         ,&
            ...
            Constructor%mat_cells, Constructor%emf, Constructor%emfm, Constructor%parallel_params, df%mat_index, df%use_GPU)
    ...
    subroutine Start_calculation(this)
        ...
        if (this%mesh%dimension == 2) then
            ...
            !$omp target enter data if(do_offload) map(to: x_target, y_target, z_target, velocity_x_target, velocity_y_target, velocity_z_target, vof_target, vol_target) &
            ...
            write(*,*) "before exit data"
            !$omp target exit data if(do_offload) map(delete: x_target, y_target, z_target, velocity_x_target, velocity_y_target, velocity_z_target, vof_target, vol_target) &
            !$omp& map(from: dvel_x_dx_target, dvel_x_dy_target, dvel_x_dz_target, dvel_y_dx_target, dvel_y_dy_target,&
            !$omp& dvel_y_dz_target, dvel_z_dx_target, dvel_z_dy_target, dvel_z_dz_target)
            write(*,*) "after exit data"
        end if
