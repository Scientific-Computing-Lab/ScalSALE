...
module steinberg_module
   use equation_of_state_module, only : equation_of_state_t
   use data_module, only : data_t
   implicit none
   private

   type, extends(equation_of_state_t), public :: steinberg_t
      private
   contains

      procedure, public :: Calculate => Calculate_steinberg
   end type steinberg_t
...
contains
   ...
   subroutine Calculate_steinberg (this, pressure, sound_vel, density, sie,&
                                      dp_de, dp_drho, dt_de, dt_drho, gamma_gas, atomic_mass, temperature, mat_num, nrg_or_tmp,&
                                      nx, ny, nz, vof, emf)

         class (steinberg_t)        , intent (inout) :: this        
      ...
      end subroutine Calculate_steinberg

end module steinberg_module
